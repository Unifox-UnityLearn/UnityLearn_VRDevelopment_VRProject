...
namespace UnityEngine.XR.Interaction.Toolkit
{
    ...
    public abstract class CustomContinuousMoveProviderBase : LocomotionProvider
    {
        ...
        protected void Update()
        {
            ...
            var input = ReadInput();
            var buttonInput = ReadButton();
            var translationInWorldSpace = ComputeDesiredMove(input, buttonInput);
            Debug.Log(translationInWorldSpace);
            ...
            switch (m_GravityApplicationMode)
            {
                ...
                case GravityApplicationMode.AttemptingMove:
                    if (input != Vector2.zero || m_VerticalVelocity != Vector3.zero || buttonInput != 0f)
                        MoveRig(translationInWorldSpace);
                    break;
                default:
        ...
        /// <summary>
        /// Reads the current value of the jump input.
        /// </summary>
        /// <returns>Returns the input value, such as from a button.</returns>
        protected abstract float ReadButton();
        ...
        protected virtual Vector3 ComputeDesiredMove(Vector2 input, float buttonInput)
        {
            if (input == Vector2.zero && buttonInput == 0f)
                return Vector3.zero;
            ...
            var inputMove = Vector3.ClampMagnitude(new Vector3(m_EnableStrafe ? input.x : 0f, buttonInput, input.y), 1f);
            ...
            translationInWorldSpace.y = buttonInput;
        ...
        protected virtual void MoveRig(Vector3 translationInWorldSpace)
        {
            ...
            if (m_CharacterController != null && m_CharacterController.enabled)
            {
                ...
                if ((m_CharacterController.isGrounded && motion.y == 0f) || !m_UseGravity || m_EnableFly)
                {
                    Debug.Log("nun");
                    m_VerticalVelocity = Vector3.zero;
                }
                else if(m_CharacterController.isGrounded && motion.y != 0f)
                {
                    m_VerticalVelocity += Vector3.up * motion.y * jumpHeight;
